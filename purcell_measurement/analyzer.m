...
xlabel('time (s)'); % Label x-axis
ylabel('counts'); % Label y-axis
title(sprintf("sum over %d on/off by chopper. Total acquire time = %d mins",normalizer,iteration-begin+1)); % Title of the plot
ylim([0 500]);


if twitch == 1
    filename = sprintf('noYBdecay(9and29and30excluded)_%d_%d.fig', begin, iteration);
    folder = 'C:\Users\Dirk\Desktop\purcell\05142024\result';
    saveas(gcf, [folder filename]);
    disp(sprintf('Saved! %s', [folder filename]));
    ...
    mat_filename = sprintf('noYBdecay(9and29and30excluded)__%d_%d.mat', begin, iteration);
